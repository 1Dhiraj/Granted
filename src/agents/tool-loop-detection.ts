...
export function detectToolCallLoop(
  ...
  // Frequency-based circuit breaker: catches spam where the same (name+args) is invoked many times
  // even when results vary slightly (different page IDs, timestamps), since `noProgressStreak`
  // requires identical results to build a streak.
  const sameArgsCount = history.reduce(
    (n, h) => (h.toolName === toolName && h.argsHash === currentHash ? n + 1 : n),
    0,
  );
  if (sameArgsCount >= resolvedConfig.globalCircuitBreakerThreshold) {
    log.error(
      `Frequency circuit breaker triggered: ${toolName} called ${sameArgsCount} times with identical args`,
    );
    return {
      stuck: true,
      level: "critical",
      detector: "global_circuit_breaker",
      count: sameArgsCount,
      message: `CRITICAL: ${toolName} has been called ${sameArgsCount} times with identical arguments. Session execution blocked by frequency circuit breaker to prevent runaway loops.`,
      warningKey: `freq:${toolName}:${currentHash}`,
    };
  }

  // Name-only frequency breaker: catches spam where args vary slightly each call.
  const sameNameCount = history.reduce(
    (n, h) => (h.toolName === toolName ? n + 1 : n),
    0,
  );
  const nameFrequencyThreshold = resolvedConfig.globalCircuitBreakerThreshold * 2;
  if (sameNameCount >= nameFrequencyThreshold) {
    log.error(
      `Name-frequency circuit breaker triggered: ${toolName} called ${sameNameCount} times`,
    );
    return {
      stuck: true,
      level: "critical",
      detector: "global_circuit_breaker",
      count: sameNameCount,
      message: `CRITICAL: ${toolName} has been called ${sameNameCount} times in this session. Session execution blocked to prevent runaway tool-spam loops.`,
      warningKey: `name-freq:${toolName}`,
    };
  }
