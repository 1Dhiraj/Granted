...
/**
 * Reset the in-process model catalog cache so the next {@link loadModelCatalog}
 * call re-builds from disk, picking up newly-saved or removed provider keys.
 */
  ...
  modelCatalogPromise = (async () => {
    ...
    try {
      ...
      if (supplemental.length > 0) {
        // Live augmentation is authoritative for its providers — evict stale models.json
        // entries for those providers so deleted models don't persist in the dropdown.
        const augmentedProviders = new Set(
          supplemental
            .map((e) => normalizeLowercaseStringOrEmpty(e.provider))
            .filter(Boolean),
        );
        const baseModels = models.filter(
          (e) => !augmentedProviders.has(normalizeLowercaseStringOrEmpty(e.provider)),
        );
        models.length = 0;
        models.push(...baseModels, ...supplemental);
      }
