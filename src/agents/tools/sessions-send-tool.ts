...
export function createSessionsSendTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Address a peer agent by id alone (no label/sessionKey) → route the
      // message straight to that agent's main session. The cross-agent policy
      // (tools.agentToAgent + visibility="all") is enforced by the visibility
      // guard below; we surface a clearer error here before that point.
      ...
      if (!sessionKey) {
        return jsonResult({
          runId: crypto.randomUUID(),
          status: "error",
          error: "Provide a target: sessionKey, label, or agentId.",
        });
      }
