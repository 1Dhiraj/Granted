...
describe("buildAssistantMessage", () => {
  ...
  // Regression tests for qwen2.5-coder and similar models that emit tool calls
  // as JSON in the content field instead of in the tool_calls array.
  describe("content-as-tool-call parsing", () => {
    const expectToolCall = (
      ...
      expect(toolCall.name).toBe(expected.name);
      expect(toolCall.arguments).toEqual(expected.arguments);
    };

    it("parses bare JSON tool call", () => {
      expectToolCall('{"name":"browser","arguments":{"action":"status"}}', {
        name: "browser",
        arguments: { action: "status" },
      });
    });

    it("parses pretty-printed JSON tool call (the qwen2.5-coder format)", () => {
      const content =
        '{\n  "name": "browser",\n  "arguments": {\n    "action": "status"\n  }\n}';
      expectToolCall(content, { name: "browser", arguments: { action: "status" } });
    });
    ...
    it("parses JSON wrapped in plain ``` fence", () => {
      const content = '```\n{"name":"browser","arguments":{"action":"start"}}\n```';
      expectToolCall(content, { name: "browser", arguments: { action: "start" } });
    });

    it("parses JSON with surrounding explanatory text", () => {
      const content =
        'I will check the browser status:\n{"name":"browser","arguments":{"action":"status"}}\nThen proceed.';
      expectToolCall(content, { name: "browser", arguments: { action: "status" } });
    });

    it("parses { name, parameters } shape (alternate format)", () => {
      expectToolCall('{"name":"browser","parameters":{"action":"snapshot"}}', {
        name: "browser",
        arguments: { action: "snapshot" },
      });
    });

    it("parses { function: { name, arguments } } shape", () => {
      expectToolCall(
        '{"function":{"name":"browser","arguments":{"action":"navigate","targetUrl":"https://google.com"}}}',
        {
          name: "browser",
          arguments: { action: "navigate", targetUrl: "https://google.com" },
        },
      );
    });

    it("parses { function: { name, arguments: stringified-json } } shape", () => {
      expectToolCall(
        '{"function":{"name":"browser","arguments":"{\\"action\\":\\"status\\"}"}}',
        { name: "browser", arguments: { action: "status" } },
      );
    });

    it("parses nested act request with ref", () => {
      const content =
        '```json\n{\n  "name": "browser",\n  "arguments": {\n    "action": "act",\n    "request": {"kind":"type","ref":"e12","text":"weather"}\n  }\n}\n```';
      expectToolCall(content, {
        name: "browser",
        arguments: {
          action: "act",
          request: { kind: "type", ref: "e12", text: "weather" },
    ...
    it("leaves plain text alone (no tool call)", () => {
      const result = buildAssistantMessage(
        {
          model: "qwen2.5-coder:14b",
          created_at: "2026-01-01T00:00:00Z",
    ...
    it("leaves non-tool-call JSON alone", () => {
      const result = buildAssistantMessage(
        {
          model: "qwen2.5-coder:14b",
          created_at: "2026-01-01T00:00:00Z",
      ...
      expect(result.stopReason).toBe("stop");
      expect(result.content).toEqual([
        { type: "text", text: '{"some":"random","data":42}' },
      ]);
    });
  });
});
...
describe("createOllamaStreamFn streaming events", () => {
  ...
  // Regression: qwen2.5-coder and similar local models stream tool calls as JSON
  // text, char-by-char. Before this fix, each char was emitted as a text_delta
  // event, so the UI rendered the JSON as a text code block even though the
  // final message was a tool call. With buffering, no text_delta events fire
  // for JSON content that resolves to a tool call.
  describe("JSON-as-tool-call buffering", () => {
    const browserTool = {
      name: "browser",
    ...
    async function streamWithTools(lines: string[]) {
    ...
    it("buffers char-by-char JSON content and emits only a done event with the tool call", async () => {
      // qwen2.5-coder streams the tool call as ~24 single-char-ish chunks.
      // Each line below represents one streaming chunk from Ollama.
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"{\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"  \\"name\\": \\"browser\\",\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"  \\"arguments\\": {\\"action\\": \\"status\\"}\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"}"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":10,"eval_count":24}',
      ];

      await withMockNdjsonFetch(lines, async () => {
        const stream = await streamWithTools(lines);
        const events = await collectStreamEvents(stream);
        const types = events.map((e) => e.type);

        // CRITICAL: no text_delta events should fire — the user must not see
        // the JSON streamed as text in the UI.
        expect(types).toEqual(["done"]);
        ...
        if (done.type === "done") {
          ...
          if (toolCall && toolCall.type === "toolCall") {
            expect(toolCall.name).toBe("browser");
            expect(toolCall.arguments).toEqual({ action: "status" });
          }
        }
      });
    });

    it("flushes the buffer as text if the JSON-looking content turns out NOT to be a tool call", async () => {
      // Model started with `{` but produced random JSON, not a tool call shape.
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"{\\"answer\\":\\"42\\"}"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":3,"eval_count":5}',
      ];
      ...
      await withMockNdjsonFetch(lines, async () => {
        ...
        // The buffer must be flushed as text so the user still sees the response.
        expect(types).toEqual(["start", "text_start", "text_delta", "text_end", "done"]);

        const delta = events.find((e) => e.type === "text_delta");
        expect(delta).toMatchObject({ delta: '{"answer":"42"}' });
    ...
    it("streams plain (non-JSON) responses normally even when tools are present", async () => {
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"Hello"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":" there"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":2,"eval_count":2}',
      ];
      ...
      await withMockNdjsonFetch(lines, async () => {
        ...
        // Plain text — should stream normally (no buffering).
        expect(types).toEqual([
          "start",
          "text_start",
          "text_delta",
    ...
    it("buffers markdown-fenced JSON tool calls", async () => {
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"```json\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"{\\"name\\":\\"browser\\",\\"arguments\\":{\\"action\\":\\"start\\"}}\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"```"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":5,"eval_count":15}',
      ];
      ...
      await withMockNdjsonFetch(lines, async () => {
        ...
        // Fence-wrapped JSON tool call: should also be buffered (no text events).
        expect(types).toEqual(["done"]);
        ...
        if (done.type === "done") {
          ...
          if (toolCall && toolCall.type === "toolCall") {
            expect(toolCall.name).toBe("browser");
            expect(toolCall.arguments).toEqual({ action: "start" });
          }
        }
      });
    });
  ...
  // Regression: qwen2.5-coder sometimes emits <|im_start|> or <|im_end|> as
  // literal text content instead of as stop tokens. This causes the turn to end
  // prematurely with an empty/garbage message, cutting off multi-step workflows.
  describe("special-token filtering", () => {
    async function streamNoTools(lines: string[]) {
      const streamFn = createOllamaStreamFn("http://ollama-host:11434");
      return streamFn(
        {
          id: "qwen2.5-coder:14b",
    ...
    it("strips <|im_start|>assistant from content and produces an empty text response", async () => {
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"<|im_start|>assistant\\n"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":5,"eval_count":3}',
      ];

      await withMockNdjsonFetch(lines, async () => {
        const stream = await streamNoTools(lines);
        const events = await collectStreamEvents(stream);
        // After stripping the special token, there is no real text content.
    ...
    it("strips <|im_end|> stop token and keeps preceding real text", async () => {
      const lines = [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"Hello there"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"<|im_end|>"},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":5,"eval_count":5}',
      ];
      ...
      await withMockNdjsonFetch(lines, async () => {
        ...
        expect(deltas.includes("<|im_end|>")).toBe(false);
      });
    });
  });
});
