...
export function describeSessionsSendTool(): string {
  return [
    "Send a message into another visible session by sessionKey, label, or agentId.",
    'To consult one of your peer agents, pass `agentId` (e.g. agentId="research-bot") with your message — it routes to that agent\'s main session, runs there, and returns its reply.',
    "Use agents_list first to discover available agent ids.",
    "Use this to delegate follow-up work or ask a specialist peer; waits for the target run and returns the updated assistant reply when available.",
  ].join(" ");
}
