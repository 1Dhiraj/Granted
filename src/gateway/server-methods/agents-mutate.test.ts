...
describe("agents.create", () => {
  ...
  it("applies model, tools, and purpose, deriving a default workspace when omitted", async () => {
    const { respond, promise } = makeCall("agents.create", {
      name: "Research Bot",
      purpose: "Gather information from the web and summarize it.",
      model: "together/moonshotai/Kimi-K2.6",
      tools: ["browser", "openclaw-web-search"],
    });
    await promise;

    // Workspace defaulted via resolveAgentWorkspaceDir when the caller omits one.
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({
        ok: true,
    ...
    // Model + tools persisted into the agent's config entry.
    ...
    // Purpose written into a workspace file (AGENTS.md) — appendFileWithinRoot
    // is called with { rootDir, relativePath, data }.
    const wrotePurpose = mocks.appendFileWithinRoot.mock.calls.some((call) =>
      call.some(
        (arg) =>
          !!arg &&
          typeof arg === "object" &&
          (arg as { relativePath?: string }).relativePath === "AGENTS.md" &&
          typeof (arg as { data?: string }).data === "string" &&
          (arg as { data: string }).data.includes("Gather information from the web"),
      ),
    );
    expect(wrotePurpose).toBe(true);
  });
