...
export const agentsHandlers: GatewayRequestHandlers = {
  ...
  "agents.create": async ({ params, respond }) => {
    ...
    // Workspace is optional from the create wizard — derive a default under the
    // agents dir when the caller doesn't pick a path.
    const requestedWorkspace = normalizeOptionalString(String(params.workspace ?? ""));
    const workspaceDir = requestedWorkspace
      ? resolveUserPath(requestedWorkspace)
      : resolveAgentWorkspaceDir(cfg, agentId);

    // Optional per-agent purpose/model/tools captured by the create wizard.
    const purpose = normalizeOptionalString(params.purpose);
    const model = normalizeOptionalString(params.model);
    const tools = Array.isArray(params.tools)
      ? params.tools.map((entry) => normalizeOptionalString(entry) ?? "").filter(Boolean)
      : undefined;
    ...
    // Write the agent's purpose into its AGENTS.md so the agent itself knows what
    // it is for — this is what makes each created agent a focused specialist.
    if (purpose) {
      if (
        !(await ensureWorkspaceFileReadyOrRespond({
          respond,
