...
export const AgentsCreateParamsSchema = Type.Object(
  {
    name: NonEmptyString,
    // Optional: when omitted the gateway derives a workspace under the agents dir
    // so the create wizard does not force the user to choose a path.
    workspace: Type.Optional(NonEmptyString),
    emoji: Type.Optional(Type.String()),
    avatar: Type.Optional(Type.String()),
    // Optional purpose written into the new agent's AGENTS.md (what this agent is for).
    purpose: Type.Optional(Type.String()),
    // Optional primary model ref for this agent (e.g. "together/moonshotai/Kimi-K2.6").
    model: Type.Optional(NonEmptyString),
    // Optional explicit tool allow-list for this agent (e.g. ["browser","desktop"]).
