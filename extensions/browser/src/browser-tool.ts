...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      "REQUIRED WORKFLOW — follow this sequence for every web task:",
      "(1) status: check if running=true.",
      "(2) start: call ONLY if running=false — launches the browser.",
      "(3) navigate or open: go to the target URL (e.g. navigate to https://google.com before touching Google).",
      "(4) snapshot: capture the live page to get real element refs (e.g. e12). ALWAYS snapshot before any act/click/type.",
      "(5) act: interact using ONLY refs returned by the most recent snapshot on that tab.",
      "CRITICAL RULES: Never use a ref without a prior snapshot. Never skip navigate/open before interacting with page elements. Refs go stale after navigation — re-snapshot whenever the page changes.",
      "Browser choice: omit profile by default for the isolated OpenClaw-managed browser (`openclaw`).",
      'For the logged-in user browser use profile="user". A Chromium-based browser (v144+) must be running.',
      "Pass targetId from the snapshot response into subsequent actions to keep the same tab.",
