...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control a web browser (navigate, read, click, type). Flow: navigate to the URL (auto-starts the browser — no status/start needed), snapshot to get element refs (e12), then act on refs from the LATEST snapshot. Re-snapshot after the page changes; never reuse stale refs. Prefer direct URLs over UI clicking. Fill whole forms in one act kind=fill fields:[{ref,value}].",
      'Default profile = the isolated OpenClaw browser; profile="user" uses the logged-in user browser (Chromium v144+). Pass targetId from a snapshot to stay on the same tab.',
