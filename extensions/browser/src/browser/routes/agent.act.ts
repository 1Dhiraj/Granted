...
function normalizeBatchAction(value: unknown): BrowserActRequest {
  ...
  switch (kind) {
    ...
    case "clickAt": {
      ...
      const y = toNumber(raw.y);
      if (x === undefined || y === undefined) {
        throw new Error("clickAt requires x and y");
      }
      const targetId = toStringOrEmpty(raw.targetId) || undefined;
      const buttonRaw = toStringOrEmpty(raw.button);
      const button = buttonRaw ? parseClickButton(buttonRaw) : undefined;
      if (buttonRaw && !button) {
        throw new Error("clickAt button must be left|right|middle");
      }
      const delayMs = normalizeBoundedNonNegativeMs(raw.delayMs, "clickAt delayMs", MAX_BATCH_CLICK_DELAY_MS);
      return {
        kind,
        x,
        y,
      ...
      run: async ({ profileCtx, cdpUrl, tab }) => {
        ...
        switch (kind) {
          ...
          case "mousemove": {
            ...
            if (isExistingSession) {
              return jsonError(res, 501, "existing-session mousemove is not supported; use Playwright profile.");
            }
            const pw = await requirePwAi(res, `act:${kind}`);
            if (!pw) {
              return;
            ...
            await pw.mouseMoveViaPlaywright({
              cdpUrl,
              targetId: tab.targetId,
              x,
              y,
          ...
          case "clickAt": {
            ...
            if (isExistingSession) {
              return jsonError(res, 501, "existing-session clickAt is not supported; use Playwright profile.");
            }
            const pw = await requirePwAi(res, `act:${kind}`);
            if (!pw) {
              return;
