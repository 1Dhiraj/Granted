...
export async function mouseMoveViaPlaywright(opts: {
  ...
  const page = await getPageForTargetId(opts);
  ensurePageState(page);
  await page.mouse.move(opts.x, opts.y, { steps: Math.max(1, Math.floor(opts.steps ?? 1)) });
}
...
export async function clickAtViaPlaywright(opts: {
  ...
  const page = await getPageForTargetId(opts);
  ensurePageState(page);
  const delayMs = resolveBoundedDelayMs(opts.delayMs ?? 0, "clickAt delayMs", MAX_CLICK_DELAY_MS);
