...
export function buildOllamaModelDefinition(
  ...
  // When capabilities is known and doesn't include "tools", explicitly disable
  // tool calling. When capabilities is undefined (show data not yet fetched),
  // leave compat unset so the default (true) applies.
  const supportsTools = capabilities === undefined ? undefined : capabilities.includes("tools");
  return {
    id: modelId,
    name: modelId,
    ...
    ...(supportsTools === false ? { compat: { supportsTools: false } } : {}),
  };
}
