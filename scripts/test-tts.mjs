// Direct test of tts.convert (returnAudio) — isolates server voice from the UI.
...
ws.once("open", async () => {
  try {
    ...
    const text = "Hello! I'm your local voice, running right here on your machine with no internet needed.";
    for (const [label, provider, ext] of [["AVA (primary)", "microsoft", "mp3"], ["PIPER (fallback)", "piper", "wav"]]) {
      const t0 = Date.now();
      const res = await call("tts.convert", { text, returnAudio: true, provider }, 60000);
      const ms = Date.now() - t0;
      if (res?.ok) {
        const p = res.payload;
        const bytes = (p.audioBase64 || "").length;
        console.log(`${label}: ${ms}ms | provider=${p.provider} | ${bytes ? Math.round(bytes*0.75/1024)+"KB audio" : "NO AUDIO"} | mime=${p.audioMime}`);
        if (p.audioBase64) writeFileSync(`d:/Dhiraj/Startup_products/openclaw/tts-${provider}.${ext}`, Buffer.from(p.audioBase64, "base64"));
      } else {
        console.log(`${label}: FAILED — ${JSON.stringify(res?.error)}`);
      }
    }
