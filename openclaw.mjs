...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `granted: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:1Dhiraj/granted#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g granted-ai@latest`.");
  return lines.join("\n");
};
