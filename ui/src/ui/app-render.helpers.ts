...
export function renderChatSessionSelect(state: AppViewState) {
  ...
  const modeToggle = renderChatSpeedToggle(state);
  ...
  return html`
    <div class="chat-controls__session-row">
      ...
      ${modeToggle} ${modelSelect} ${thinkingSelect}
    </div>
  `;
}

// Option-1 cost routing: sessions default to the free/fast model
// (agents.defaults.model.primary); this toggle escalates the current chat
// session to the advanced (paid) model on demand via the same session model
// override the model picker uses.
const ADVANCED_CHAT_MODEL_REF = "together/moonshotai/Kimi-K2.6";

function isAdvancedChatModelValue(value: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(value);
  if (!normalized) {
    return false;
  }
  const advancedRef = ADVANCED_CHAT_MODEL_REF.toLowerCase();
  // Session overrides may be stored qualified ("together/moonshotai/Kimi-K2.6")
  // or as the bare model id ("moonshotai/Kimi-K2.6").
  return normalized === advancedRef || advancedRef.endsWith(`/${normalized}`);
}

function renderChatSpeedToggle(state: AppViewState) {
  const { currentOverride } = resolveChatModelSelectState(state);
  const advanced = isAdvancedChatModelValue(currentOverride);
  const busy =
  ...
  `;
  const activeStyle = `
    background:var(--accent-subtle,#1a2a3a);
  ...
  return html`
    ...
    >
      <button
        title="Fast — default free model"
        aria-pressed=${!advanced}
        ?disabled=${disabled}
        style="${baseStyle}${advanced ? "" : activeStyle}"
        @click=${async () => {
          if (!currentOverride) {
            return;
          }
      ...
      <button
        ...
        aria-pressed=${advanced}
        ?disabled=${disabled}
        style="${baseStyle}border-left:1px solid var(--border,#333);${advanced ? activeStyle : ""}"
        @click=${async () => {
          if (advanced) {
            return;
          }
...
function renderChatModelSelect(state: AppViewState) {
  ...
  const query = state.chatModelSearchQuery ?? "";
  const filtered = query.trim()
    ? options.filter((e) => e.label.toLowerCase().includes(query.toLowerCase()))
    : options;

  const closeDropdown = () => {
    state.chatModelSearchOpen = false;
    state.chatModelSearchQuery = "";
  };

  return html`
    <div class="chat-model-search-wrap" style="position:relative;">
      <button
        class="chat-model-search-trigger field"
        title=${selectedLabel}
        ?disabled=${disabled}
        style="
          display:inline-flex;
        ...
        @click=${(e: Event) => {
          ...
          state.chatModelSearchOpen = !state.chatModelSearchOpen;
          state.chatModelSearchQuery = "";
          if (state.chatModelSearchOpen) {
            // store position for fixed dropdown
            btn.dataset.dropdownTop = String(rect.bottom + 6);
            btn.dataset.dropdownLeft = String(rect.left);
            btn.dataset.dropdownWidth = String(Math.max(rect.width, 260));
            setTimeout(() => {
              const el = document.querySelector(".chat-model-search-input") as HTMLInputElement | null;
              el?.focus();
            }, 50);
          }
        }}
      ...
      ${state.chatModelSearchOpen
        ? html`
          <div
            class="chat-model-search-dropdown"
            style="
              position:fixed;
              top:${(document.querySelector(".chat-model-search-trigger") as HTMLElement)?.getBoundingClientRect().bottom + 6 || 60}px;
              left:${(document.querySelector(".chat-model-search-trigger") as HTMLElement)?.getBoundingClientRect().left || 0}px;
              z-index:9999;
              background:var(--panel, #1e1e2e);
              border:1px solid var(--border, #333);
              border-radius:var(--radius, 6px);
              box-shadow:0 4px 16px rgba(0,0,0,0.4);
              min-width:${Math.max((document.querySelector(".chat-model-search-trigger") as HTMLElement)?.getBoundingClientRect().width || 0, 260)}px;
              max-width:360px;
              display:flex;
          ...
          >
            <div style="padding:8px;">
              <input
                ...
                "
                @input=${(e: Event) => {
                  state.chatModelSearchQuery = (e.target as HTMLInputElement).value;
                }}
            ...
            <div style="max-height:240px;overflow-y:auto;">
              <div
                style="
                  ...
                  font-size:13px;
                  color:var(--text-muted, #aaa);
                  ${currentOverride === "" ? "background:var(--accent-subtle,#1a2a3a);color:var(--accent,#4a9eff);" : ""}
                "
                @click=${async () => {
                  await switchChatModel(state, "");
                  closeDropdown();
                ...
                (entry) => html`
                  <div
                    style="
                      ...
                      overflow:hidden;
                      text-overflow:ellipsis;
                      ${entry.value === currentOverride ? "background:var(--accent-subtle,#1a2a3a);color:var(--accent,#4a9eff);" : ""}
                    "
                    title=${entry.label}
                    @mouseenter=${(e: Event) => {
                      if (entry.value !== currentOverride)
                        (e.currentTarget as HTMLElement).style.background = "var(--bg-muted,#111)";
                    }}
                    @mouseleave=${(e: Event) => {
                      if (entry.value !== currentOverride)
              ...
              ${filtered.length === 0
                ? html`<div style="padding:12px;color:var(--text-muted,#aaa);font-size:13px;text-align:center;">No models found</div>`
                : nothing}
            </div>
          </div>
