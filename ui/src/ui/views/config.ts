...
const THEME_OPTIONS: ThemeOption[] = [
  { id: "claw", label: "Granted", description: "Emerald & graphite", icon: icons.zap },
  { id: "knot", label: "Noir", description: "Black & red", icon: icons.link },
  { id: "dash", label: "Blueprint", description: "Warm chocolate", icon: icons.barChart },
];
...
export function renderConfig(props: ConfigProps) {
  ...
  const browserSchemaMode: string =
    (
      (
        ((props.formValue as Record<string, unknown> | null)?.["tools"] as
          | Record<string, unknown>
          | undefined)?.["browser"] as Record<string, unknown> | undefined
      ...
      <main class="config-main">
        ...
        ${activeSectionMeta && formMode === "form"
          ? html`
            ...
            `
          : nothing}
        <!-- Browser settings toggle panel (tools.browser section) -->
        ${props.activeSection === "tools" && formMode === "form"
          ? html`
              <div style="padding: 16px 24px; background: var(--panel); border: 1px solid var(--border); border-radius: var(--radius-lg, 8px); margin-bottom: 16px;">
                <div style="margin-bottom: 12px;">
                  <h3 style="margin: 0 0 4px 0; font-size: 14px; font-weight: 600; color: var(--text-strong, var(--text));">Browser Tool Schema</h3>
                  <p style="margin: 0; font-size: 12px; color: var(--text-muted);">
                    Choose schema based on your LLM model
                  </p>
                </div>

                <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                  <button
                    style="${browserSchemaMode === 'full'
                      ? 'background: var(--accent); color: var(--primary-foreground, #fff); border: 1px solid var(--accent);'
                      : 'background: var(--bg-muted); color: var(--text); border: 1px solid var(--border);'} padding: 8px 16px; border-radius: var(--radius-sm, 4px); cursor: pointer; font-size: 12px; font-weight: 500; transition: all 200ms;"
                    @click=${() => {
                      props.onFormPatch(['tools', 'browser', 'schema'], 'full');
                    }}
                  >
                    Full (30+ params)
                  </button>
                  <button
                    style="${browserSchemaMode === 'simplified'
                      ? 'background: var(--accent); color: var(--primary-foreground, #fff); border: 1px solid var(--accent);'
                      : 'background: var(--bg-muted); color: var(--text); border: 1px solid var(--border);'} padding: 8px 16px; border-radius: var(--radius-sm, 4px); cursor: pointer; font-size: 12px; font-weight: 500; transition: all 200ms;"
                    @click=${() => {
                      props.onFormPatch(['tools', 'browser', 'schema'], 'simplified');
                    }}
                  >
                    Simplified (4 params)
                  </button>
                ...
                <div style="font-size: 11px; color: var(--text-muted); line-height: 1.5;">
                  <div style="margin-bottom: 4px;">
                    <strong style="color: var(--text);">Full:</strong> Best for Claude, GPT. Supports all advanced browser operations.
                  </div>
                  <div>
                    <strong style="color: var(--text);">Simplified:</strong> Best for Ollama, NVIDIA, local models. Simple schema, more tool calls.
                  </div>
                </div>
              </div>
            `
