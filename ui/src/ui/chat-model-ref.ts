...
export function buildQualifiedChatModelValue(model: string, provider?: string | null): string {
  ...
  // If the model id already starts with this provider's prefix, treat it as
  // already qualified. Otherwise always prepend the explicit provider so model
  // ids that contain "/" (e.g. "Qwen/Qwen2.5-7B" on Together AI) don't get
  // mis-parsed as having "Qwen" as the provider.
  if (trimmedProvider && trimmedModel.toLowerCase().startsWith(`${trimmedProvider.toLowerCase()}/`)) {
    return trimmedModel;
  }
