...
const lazyProviders = createLazy(() => import("./views/providers.ts"));
...
const lazyWorld = createLazy(() => import("./views/world.ts"));
    ...
    >
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${state.tab === "agents"
          ? lazyRender(lazyAgents, (m) =>
              m.renderAgents({
                ...
                create: {
                  ...
                  purpose: state.agentCreatePurpose,
                  tools: state.agentCreateTools,
                  model: state.agentCreateModel,
                  busy: state.agentCreateBusy,
                ...
                onCreateOpen: () => {
                  ...
                  state.agentCreateTools = [];
                  state.agentCreateModel = "";
                ...
                onCreateNameChange: (name) => (state.agentCreateName = name),
                onCreatePurposeChange: (purpose) => (state.agentCreatePurpose = purpose),
                onCreateModelChange: (model) => (state.agentCreateModel = model),
                onCreateToolToggle: (toolId, enabled) => {
                  const set = new Set(state.agentCreateTools);
                  if (enabled) {
                    set.add(toolId);
                  } else {
                    set.delete(toolId);
                  }
                  state.agentCreateTools = [...set];
                },
                onCreateSubmit: () => {
                  void (async () => {
                    if (!state.client || !state.connected) {
                      state.agentCreateError = "Not connected to the gateway.";
                      return;
                    }
                    const name = state.agentCreateName.trim();
                    if (!name) {
                      state.agentCreateError = "Name is required.";
                      return;
                    }
                    ...
                    try {
                      ...
                      const purpose = state.agentCreatePurpose.trim();
                      if (purpose) {
                        params.purpose = purpose;
                      }
                      if (state.agentCreateModel) {
                        params.model = state.agentCreateModel;
                      }
                      if (state.agentCreateTools.length > 0) {
                        params.tools = state.agentCreateTools;
                      }
          ...
          : nothing}
        ${state.tab === "providers"
          ? lazyRender(lazyProviders, (m) =>
              m.renderProviders({
                connected: state.connected,
                loading: state.providersLoading,
                providers: state.providersList,
                error: state.providersError,
                edits: state.providersEdits,
                openId: state.providersOpenId,
                savingId: state.providersSavingId,
                messages: state.providersMessages,
                ollamaModels: state.ollamaModels,
                ollamaLoading: state.ollamaLoading,
                ollamaError: state.ollamaError,
                onRefresh: () => void loadProviders(state),
                onEditChange: (id, value) => {
                  state.providersEdits = { ...state.providersEdits, [id]: value };
                },
                onToggleOpen: (id) => {
                  state.providersOpenId = state.providersOpenId === id ? null : id;
                },
                onSave: (id) => {
                  void saveProviderKey(state, id).then(() => {
                    if (state.client) {
                      void loadModels(state.client).then((models) => {
                ...
                onDelete: (id) => {
                  void deleteProviderKey(state, id).then(() => {
                    if (state.client) {
                      void loadModels(state.client).then((models) => {
                        state.chatModelCatalog = models;
                      });
                ...
                onTestOllama: (providerId: string) => {
                  void listOllamaModels(state, providerId).then(() => {
                    if (state.client) {
                      void loadModels(state.client).then((models) => {
                        state.chatModelCatalog = models;
                      });
        ...
        ${state.tab === "chat"
          ? renderChat({
              ...
              onSendDirect: (text: string) => void state.handleSendChat(text),
        ...
        ${state.tab === "dreams"
          ...
          : nothing}
        ${state.tab === "world"
          ? lazyRender(lazyWorld, (m) => {
              const switchToAgent = (agentId: string, loadHistory: boolean) => {
                const targetKey = buildAgentMainSessionKey({ agentId });
                if (state.sessionKey === targetKey) {
                  return;
                }
              ...
              return m.renderWorld({
                ...
                onPromptInput: (value: string) => {
                  state.worldPrompt = value;
                },
                onSend: () => {
                  const agentId = state.worldSelectedAgentId;
                  const text = state.worldPrompt.trim();
                  if (!agentId || !text) {
                    return;
                  }
                  switchToAgent(agentId, false);
                  state.worldPrompt = "";
